(() => {
  ...
  async function exportPptx() {
    await withExportState("PPTX 파일을 만들고 있어요…", async () => {
      ...
      pptx.defineLayout({ name: "A4", width: 8.2677165, height: 11.692913 });
      pptx.layout = "A4";
      ...
      pages.forEach((page, index) => {
        ...
        slide.addImage({
          data: svgToDataUri(page.svg),
          x: 0,
          y: 0,
          w: 8.2677165,
          h: 11.692913,
          altText: `${title} ${index + 1}쪽`,
        });
      });
      ...
      return `PPTX ${pages.length}장을 저장했어요.`;
    });
  }
  ...
  function serializePage(page, pageIndex) {
    ...
    return { width, height, content, svg };
  }
  ...
  function appendTextNode(node, context, output, usedBullets) {
    ...
    if (parent.closest("li") && !usedBullets.has(parent.closest("li"))) {
      ...
      const first = lines[0];
      output.push(
        `<text x="${round(first.x - fontSize * 0.82)}" y="${first.y}"`,
  ...
  function measureTextLines(node, context) {
    ...
    for (let index = 0; index < text.length; index += 1) {
      ...
      if (!current || Math.abs(current.top - top) > 1.5) {
        current = { x, top, y: bottom - Math.max(0.5, (bottom - top) * 0.12), text: char };
        lines.push(current);
      } else {
        current.text += char;
      }
